...
def main():
    ...
    print(f"开始自动训练所有数据类型、扬声器和麦克风配置")
    ...
    for data_type_val in data_type_values:
        print(f"\n{'='*80}")
        print(f"正在训练数据类型: {data_type_val}")
        print(f"{'='*80}")
        ...
        for num_x_val in num_x_values:
            print(f"\n{'='*60}")
            print(f"正在训练 {num_x_val} 个麦克风配置 (data_type = {data_type_val})")
            print(f"{'='*60}")
            ...
            for idx in range(6):  # 0到5共6个扬声器
                print(f"\n{'-'*40}")
                print(f"正在训练扬声器 {idx} (data_type = {data_type_val}, num_x = {num_x_val}, src_idx = {idx})")
                print(f"{'-'*40}")
                ...
                try:
                    result = subprocess.run([sys.executable, 'main.py'], 
                                          ...
                                          text=True, 
                                          check=True)
                    print(f"扬声器 {idx} (data_type = {data_type_val}, num_x = {num_x_val}) 训练完成!")
                except subprocess.CalledProcessError as e:
                    print(f"扬声器 {idx} (data_type = {data_type_val}, num_x = {num_x_val}) 训练失败: {e}")
                    print("是否继续训练下一个扬声器? (y/n): ", end='')
                    choice = input().lower()
                    if choice != 'y':
                        print("训练中断")
                        return
